...
* На входе — предохранитель F1 5 А и диод Шоттки D1 от переполюсовки. Насос — через отдельный F2 3 А.
...
площадкой — **+12 В**, второй — **GND (минус)**. Перепутанная полярность не сожжёт плату: диод D1 просто не
пропустит ток, и плата не включится.
...
| D1 | диод 1N5822 | защита от переполюсовки |
...
| R4 | 10 кОм | подтяжка кнопки |
| R5, R6 | 10 кОм, 20 кОм | делитель сигнала расходомера 5 → 3,3 В |
| R7 | 10 кОм | подтяжка датчика дождя |
| R8 | 1 кОм | защита входа датчика дождя |
| SW1 | кнопка | сброс настроек |
...
| Резисторы, керамический C4 | 0,6 мм | 0,8 мм | 1,6 мм |
...
| D1 | диод Шоттки 3 А | `1N5822` | 1 |
| C1 | 1000 мкФ 16 В, шаг 5 мм | | 1 |
| C2, C3 | 100 мкФ 10 В, шаг 2,5 мм | | 2 |
| R1–R4 | 10 кОм 0,25 Вт (подтяжки S0…S2 вниз, кнопка вверх) | | 4 |
| R5, R6 | 10 кОм и 20 кОм (делитель расходомера 5→3,3 В) | | 1+1 |
...
* Выход YF-S201 пятивольтовый: R5/R6 делят его до 3,3 В перед GPIO27.
