...
void valveClose(int index) {
  ...
  // 💪 Пересчитать состояние насоса (пусковое окно и дренаж учитывает сам хелпер)
  stopPumpIfNeed();
...
void stopPumpIfNeed() {
  ...
  // ⏱️ Пусковой режим: после открытия клапана насос обязан работать минимум
  // PUMP_TIMEOUT секунд (pumpStart обнуляет handlePumpTimeout в основном цикле,
  // после чего решает порог ниже). Без этой проверки завершение пролива дренажа
  // (spillageTick) или смена настройки boostPumpValves выключали насос раньше
  // времени — типичный случай: после перезагрузки первое открытие клапана
  // запускало пролив, и его завершение глушило насос на 12-й секунде.
  if (pumpStart != 0) {
    digitalWrite(PUMP, HIGH);
    return;
  }
