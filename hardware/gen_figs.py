...
# ряд 1: гнёзда
x = PX + 20; y = PY + 62
for key, t, ln, col in [('esp', 'ESP32 DevKitC', ['гнездо 2×19'], BLUE), ('pcf', 'PCF8574P', ['DIP-16'], AMBER),
                        ('mux', '74HC4051', ['DIP-16'], AMBER), ('rtc', 'DS3231', ['гнездо 1×6'], BLUE), ('sd', 'SD шилд', ['гнездо 2×8'], BLUE)]:
    b = c.box(key, x, y, t, ln, *col, tsize=13, lsize=11, pad=10); x += b[2] + 10
assert x - 10 <= PX + PW - 20, 'ряд 1 шире платы: %d' % x
# ряд 2
x = PX + 20; y += 80
for key, t, ln, col in [('dc5', 'DC-DC 12→5 В', ['TO-220'], PINK), ('dc3', 'DC-DC 5→3,3 В', ['TO-220'], PINK),
                        ('btn', 'Кнопка сброса', ['6×6'], BLUE), ('fuse', 'Защита входа', ['F1 · F2 · Q1 · TVS'], GREY)]:
    b = c.box(key, x, y, t, ln, *col, tsize=13, lsize=11, pad=10); x += b[2] + 10
assert x - 10 <= PX + PW - 20, 'ряд 2 шире платы: %d' % x
# ряд 3: паяемое
y += 80
c.box('solder', PX + 20, y, 'Паяется на плату', ['держатели F1/F2, MOSFET, TVS, 6 конденсаторов, 11 резисторов, кнопка, штыри, гнёзда, панельки, клеммники'],
      *GREY, tsize=13, lsize=11, w=PW - 40)
# ряд 4: клеммники
y += 76
b1 = c.box('term12', PX + 20, y, 'Клеммники 5,08 мм 2P ×5', ['12 В вход · 12 В клапаны · 12 В насос', 'фоторезистор · датчик дождя'], *GREEN, tsize=13, lsize=11)
...
assert b2[0] + b2[2] <= PX + PW - 20, 'ряд 4 шире платы: %d' % (b2[0] + b2[2])
# ряд 5
y += max(b1[3], b2[3]) + 16
b3 = c.box('term3', PX + 20, y, 'Клеммники 5,08 мм 3P ×10', ['8 × датчик влажности (3V3/5V по JP1 · GND · AOUT)', 'расходомер · реле насоса'], *GREEN, tsize=13, lsize=11, w=PW - 40)
c.text(PX + PW / 2, PY + PH - 16, 'Ни одного провода «навесом»: все связи между модулями — дорожки платы', 13, '#1B5E20', bold=True)

# внешние блоки слева
psu = c.box('psu', 40, 120, 'Источник 12 В 5 А', ['на клеммник J1'], *PINK, tsize=13, lsize=11)
c.line([right(psu), (PX, right(psu)[1])], '#D81B60', 4, arrow=True)
r8 = c.box('r8', 40, 330, 'Реле 8 кан. 5 В, H/L', ['клапаны V1…V8', 'перемычки в положении L'], *ORANGE, tsize=13, lsize=11)
r4 = c.box('r4', 40, 440, 'Реле 1 + 2 кан. 5 В, H/L', ['насос · налив · дренаж', 'перемычки в положении H'], *ORANGE, tsize=13, lsize=11)
ib = c.boxes['idc']
...
c.text(r8[0] + 4, (r8[1] + r8[3] + r4[1]) / 2 + 4, 'провода к клеммам реле', 11, '#E65100', 'start')

# внешние блоки справа
RX = PX + PW + 60
s1 = c.box('soil', RX, 120, 'Датчики влажности ×8', ['ёмкостные v2.0, 3 провода'], *CYAN, tsize=13, lsize=11)
s2 = c.box('rain', RX, 210, 'Дождь · Свет', ['дождь — сухой контакт, как кнопка', 'свет — выносной фоторезистор'], *CYAN, tsize=13, lsize=11)
s3 = c.box('flow', RX, 300, 'Расходомер YF-S201', ['5 В; делитель на плате'], *CYAN, tsize=13, lsize=11)
# нагрузки 12 В — слева, под своими реле: провода от контактов реле вниз
v = c.box('valves', 40, 640, 'Клапаны 12 В ×8', ['на контакты реле 8 кан.'], *PURPLE, tsize=13, lsize=11)
pm = c.box('pump', 40, 720, 'Насос · налив · дренаж 12 В', ['на контакты реле 1 + 2 кан.'], *PURPLE, tsize=13, lsize=11)
for b in (psu, r8, r4, v, pm):
    assert b[0] + b[2] <= PX - 40, 'левый блок упирается в плату'
for b in (s1, s2, s3):
    assert b[0] + b[2] <= c.w - 20, 'правый блок за краем листа'
t3 = c.boxes['term3']
xr = PX + PW
xs = RX - 28
for b in (s1, s2, s3):
...
# силовые провода 12 В: два пунктира вдоль левого края, каждый от своего реле к своей нагрузке
c.line([(r8[0], right(r8)[1]), (26, right(r8)[1]), (26, left(v)[1]), left(v)], '#8E24AA', 3, dash='6 4')
c.line([(r4[0], right(r4)[1]), (14, right(r4)[1]), (14, left(pm)[1]), left(pm)], '#8E24AA', 3, dash='6 4')
c.text(40, v[1] - 14, 'силовые провода 12 В — от контактов реле, минуя плату', 11, '#6A1B9A', 'start')
...
c.note(40, 720, 1120, [
    ...
    '• между платой и реле — провода в винтовых клеммах с обеих сторон, между платой и реле никаких Dupont.'], 'Как ничего не отваливается')
