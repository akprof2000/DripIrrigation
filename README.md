...
Готовая печатная плата, в которую все модули вставляются в гнёзда, а внешние провода заходят на винтовые
клеммники: [описание платы](docs/hardware-carrier-board.md), [список покупок](docs/bom.md),
[пошаговая сборка](docs/assembly.md). Исходники платы и генератор — в папке `hardware/`.
...
3. Токен бота можно задать двумя способами (начиная с v1.8.0):
   - **ничего не делать** — при первом запуске устройство само попросит токен на странице настройки WiFi и сохранит его в EEPROM (так работают готовые бинарники из релизов);
   - **вшить в прошивку** — скопировать `secrets.example.h` в `secrets.h` и вписать токен (файл не коммитится в git).
...
1. **Настройка Wi-Fi.** При первом старте устройство поднимает собственную Wi-Fi точку. Подключитесь к ней — откроется страница настройки: выберите домашнюю сеть, введите пароль и **токен бота** (если он не вшит в прошивку). После сохранения устройство перезагрузится.

   > 🔐 Токен хранится в EEPROM. Если он уже сохранён, поле «Токен» можно оставить пустым — значение не изменится. А если прошивка окажется без токена (например, взята из релизов), устройство при старте поднимет портал **только для его ввода**: сеть, пользователи и калибровки при этом сохраняются, и работа продолжится с того же места.
...
### Аварийный режим при неисправном оборудовании

Платы (RTC, SD-карта, расширитель клапанов PCF8574) инициализируются **неблокирующе**: если узел не отвечает, система всё равно завершает загрузку и поднимает бота. Неисправность приходит в чат с пояснением последствий — не нужно ехать к устройству и подключаться кабелем, чтобы понять, что случилось.

Пока неисправность не устранена:

- **полив остановлен** — работать с неисправным железом опасно;
- насос, дренаж и залив бака **выключены напрямую по пинам** (не через возможно мёртвый PCF8574);
- интерфейс бота свёрнут до отчёта и **единственной кнопки «Перезагрузка»** — починив контакт, систему можно поднять прямо из чата.
...
1. Возьмите готовый файл со [страницы релизов](https://github.com/akprof2000/DripIrrigation/releases) (собирается автоматически в GitHub Actions) — или соберите сами (`Скетч → Экспорт бинарного файла` в Arduino IDE):
   - `DripIrrigation.ino.bin` — обычный (~1.3 МБ);
   - `DripIrrigation.ino.bin.gz` — сжатый (~0.8 МБ), удобнее по мобильному интернету — устройство распакует его само.
2. Отправьте файл в чат боту **от имени владельца**
3. Устройство скачает файл, обновится и перезагрузится; после старта придёт сообщение «🚀 Система запущена! Прошивка vX.Y.Z»

> 💡 Имя файла может содержать суффикс копии, который добавляет Windows при повторном скачивании: `DripIrrigation.ino.bin (2).gz` и `DripIrrigation.ino (2).bin` тоже принимаются — переименовывать не нужно.

> 🔁 Если пришло `OTA Fetch Error` — файл не удалось **скачать** (обычно не хватило памяти на защищённое соединение после долгой работы). Прошивка при этом не пострадала: просто отправьте файл ещё раз, а если повторяется — перезагрузите систему через меню и повторите.

> ✅ Проверить версию можно в любой момент — она указана в шапке `/status`. Если после OTA номер не изменился, обновление не применилось.

> 🛡️ OTA безопасен: прошивка пишется в неактивный раздел, загрузка переключается только при валидном образе. Неудачное обновление оставит устройство на прежней прошивке (не «кирпич»).

> ⚠️ Прошивка должна быть собрана под ту же схему разделов (partition scheme), что и установленная.

> 🏷️ Версия задаётся в `version.h` (`FW_VERSION`) и должна совпадать с git-тегом релиза.

## Сторонние компоненты

- Библиотеки [Alex Gyver](https://github.com/GyverLibs): FastBot2, GyverNTP, GyverDS3231, FileData, CharDisplay и зависимости
- [uzlib](https://github.com/pfalcon/uzlib) (zlib-лицензия) — потоковая распаковка gzip для OTA из `.gz` (файлы `uzlib.h`, `tinflate.c`, `tinfgzip.c`, `crc32.c`, `adler32.c` и заголовки)
- PCF8574 library (Renzo Mischianti)
