...
// 🕐 Поиск RTC: сколько попыток и с какой паузой. Ограниченное число попыток
//    вместо бесконечного ожидания — загрузка обязана дойти до botInit(),
//    иначе сообщить об аварии будет некому.
#define RTC_INIT_ATTEMPTS 3
#define RTC_INIT_RETRY_MS 300
...
void setup() {
  ...
  // 🌱 Инициализация модулей. Порядок важен: сначала железо, потом бот —
  //    тогда стартовое сообщение в чат уже знает, что именно не поднялось.
  //    Ни один из этих вызовов не блокирует загрузку: неответившая плата
  //    отмечается в реестре (faults.h), и мы идём дальше.
  systemInit();  // 📡 WiFi, EEPROM, SD, датчики
  valvesInit();  // 🚰 Клапаны через PCF8574

  // 🕐 Инициализация RTC (часы реального времени) — неблокирующая.
  //    Раньше здесь был бесконечный `while (!rtc.begin())`: без часов прошивка
  //    зависала до сброса, loop() не стартовал, бот молчал, и понять причину
  //    можно было, только подключившись к устройству.
  bool rtcOk = false;
  for (int attempt = 1; attempt <= RTC_INIT_ATTEMPTS; attempt++) {
    if (rtc.begin()) {
      rtcOk = true;
      break;
    }
    LOG_E("RTC не найден (попытка %d из %d)", attempt, RTC_INIT_ATTEMPTS);
  ...
  if (rtcOk) {
    ...
    hwSetFault(HW_RTC, "не отвечает на шине I2C");
  }

  botInit();  // 🤖 Telegram-бот — последним, чтобы сразу сообщить об авариях
...
// ============================================================
// 🛑 Гашение нагрузок при переходе в аварийный режим
// ============================================================
// Вызывается из loop() на каждой итерации, но работу делает один раз: важно
// не оставить открытым клапан или включённым насос, если авария случилась
// посреди полива. Закрытие клапанов — «по возможности»: если не поднялся сам
// PCF8574, команда не дойдёт, поэтому реле насоса и дренажа гасим напрямую
// по пинам, не полагаясь на расширитель портов.
static bool faultShutdownDone = false;

static void faultSafeShutdown() {
  if (faultShutdownDone) return;
...
void loop() {
  ...
  // 🆘 Аварийный режим: какая-то плата не поднялась при старте.
  //    Полив не ведём — без часов расписание бессмысленно, без карты некуда
  //    писать журнал, без PCF8574 клапанами вообще не управлять. Зато бот и
  //    watchdog живут: в чате виден список неисправностей, а кнопка
  //    «Перезагрузка» позволяет поднять систему, не подключаясь кабелем.
  if (hwHasFault()) {
    faultSafeShutdown();   // 🛑 один раз гасим нагрузки
    hwReportToChat();      // 📨 отчёт в чат (если при старте связи ещё не было)
    esp_task_wdt_reset();  // 🐕 иначе watchdog уронит устройство через 5 минут
    return;
  }
