...
// В этих режимах датчик держат в воде / на воздухе — показания заведомо
// нерабочие, поэтому авто-расширение границ калибровки должно быть отключено,
// иначе оно «выучит» калибровочные экстремумы как рабочий диапазон.
bool calibrationInProgress() {
  for (uint8_t i = 0; i < userCount; i++) {
    int a = users[i].action;
    // авто-калибровка: старт → вода → сухо → завершение (1100..1137)
    if (a >= Dlg::CalibStart && a <= Dlg::CalibFinish + Dlg::Last) return true;
    // ручная калибровка (1800..1807)
    if (a >= Dlg::CalibManual && a <= Dlg::CalibManual + Dlg::Last) return true;
    // поиск датчика: замер в воде и определение (3000..3020)
    if (a >= Dlg::SearchStart && a <= Dlg::SearchDetect) return true;
  }
  return false;
}
