...
public:
  ...
  // 🔧 Авто-расширение границ по фактическому показанию: если текущее значение
  // вышло за границу, заданную при калибровке, граница отодвигается до нового
  // значения с запасом «дельта калибровки» (_border). Возвращает true, если
  // границы изменились (нужно сохранить в конфиг). Сужения границ нет —
  // только расширение. Значения ограничены диапазоном АЦП 0..4095, потому что
  // в Config границы хранятся как uint16_t (отрицательное значение обернулось бы).
  bool autoExtend(int index) {
    bool changed = false;
    int val = _curr[index];
    if (val < _low[index]) {                 // 💧 «влажнее», чем калибровочная вода
      int nl = val - _border;
      _low[index] = (nl < 0) ? 0 : nl;
      changed = true;
    }
    if (val > _high[index]) {                // 🌵 «суше», чем калибровочное сухое
      int nh = val + _border;
