...
# ───────────── 1. Структура ─────────────
c = Canvas(1400, 800, 'Из чего собирается контроллер',
...
# ряд 5
y += max(b1[3], b2[3]) + 16
b3 = c.box('term3', PX + 20, y, 'Клеммники 5,08 мм 3P ×10', ['8 × датчик влажности (3V3 или 5V по JP1 · GND · AOUT)', 'расходомер · реле насоса'], *GREEN, tsize=13, lsize=11, w=PW - 40)
...
c.text(PX - 36, right(r8)[1] + 40, 'провода к', 11, '#E65100', 'end')
c.text(PX - 36, right(r8)[1] + 54, 'клеммам реле', 11, '#E65100', 'end')

# внешние блоки справа
RX = PX + PW + 60
s1 = c.box('soil', RX, 120, 'Датчики влажности ×8', ['ёмкостные v2.0, 3 провода'], *CYAN, tsize=14, lsize=11)
...
v = c.box('valves', 40, 640, 'Клапаны 12 В ×8', ['на контакты реле 8 кан.'], *PURPLE, tsize=14, lsize=11)
pm = c.box('pump', 40, 720, 'Насос · налив · дренаж 12 В', ['на контакты реле 1 + 2 кан.'], *PURPLE, tsize=14, lsize=11)
t3 = c.boxes['term3']
xr = PX + PW
xs = RX - 28
for b in (s1, s2, s3):
