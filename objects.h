...
// 🔐 Токен бота в EEPROM. Раньше он был только в secrets.h, то есть компилировался
// внутрь прошивки — из-за этого нельзя опубликовать универсальный бинарник
// (в сборке из CI оказывалась заглушка, и устройство не выходило на связь).
// Теперь токен вводится на странице настройки WiFi и живёт в EEPROM.
// Свободный участок EEPROM: 104..239 (до 100 — WiFi, с 240 — версия и пользователи).
#define EEPROM_TOKEN_ADDR 110  // 📍 адрес токена бота
#define BOT_TOKEN_LEN 64       // 📏 буфер токена (реальный ~46 символов)

// 📮 Позиция в очереди обновлений Telegram (update_id + 1), пережившая перезагрузку.
// Telegram удаляет апдейт из очереди только когда бот запросит следующий пакет с
// бо́льшим offset. Если устройство перезагрузилось сразу после команды (OTA!),
// подтверждения не было — и после старта прилетает то же сообщение с прошивкой,
// то есть обновление зацикливается. Поэтому offset сохраняем в EEPROM.
#define EEPROM_UPD_ID_ADDR 180

extern char botToken[BOT_TOKEN_LEN];  // 🤖 действующий токен (EEPROM → иначе secrets.h)

// ✅ Похоже ли на настоящий токен Telegram (цифры «:» буквы), а не на заглушку
